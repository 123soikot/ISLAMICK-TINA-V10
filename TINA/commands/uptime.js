...
const getSystemStats = () => {
  ...
  const memoryUsage = (process.memoryUsage().rss / (1024 ** 2)).toFixed(2) + ' MB';
  const cores = os.cpus().length;
  const platform = os.platform();
  const arch = os.arch();
  const cpuUsage = 'N/A'; // উন্নত CPU ব্যবহার পরিমাপ প্রয়োজন
  const ping = '1ms'; // নির্দিষ্ট মান
...
module.exports.config = {
  ...
  description: "System uptime and information.",
